...
RUN go build -ldflags="-X 'main.Version=v2.1.6'"
...
RUN { \
    echo "*/10 0-5,9-23 * * * cd /app/helperr && ./helperr relocate >> /var/log/cron.log 2>&1"; \
    echo "0 7 * * * cd /app/helperr && ./helperr rename >> /var/log/cron.log 2>&1"; \
    echo "0 8 * * * cd /app/helperr && ./helperr missing >> /var/log/cron.log 2>&1"; \
    echo "0 20 * * * cd /app/helperr && ./helperr missing >> /var/log/cron.log 2>&1"; \
} > /etc/crontabs/root
